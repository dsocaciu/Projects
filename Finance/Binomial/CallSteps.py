#Step 5 - User input for European Put/Call, continuous as per Black-Scholes model
...
for i in reversed(sorted(CallOptionLattice)):
	...
	for j in CallOptionLattice[i]:
		if i == len(CallOptionLattice)-1:
			...
			j.Price = round((q*j.Up.Price + one_minus_q * j.Down.Price)/exp(rate*(Tyears/periods)),3)
			#j.Price = round(max())
			#print(str(j.Price))
