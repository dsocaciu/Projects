#Step 7 - User input for American Put/Call on Forwards, continuous as per Black-Scholes model
...
if len(sys.argv) > 1:
	if sys.argv[1]  == 'e' :
		...
		S0 = float(S0)
		K = input("Enter the strike price (default 100): ")
		K = float(K)
		Tyears = input("Enter the time period (default .25): ")
		Tyears = float(Tyears)
		sigma = input("Enter the volatilty (default 30%): ")
		sigma = float(sigma)/100.0
		periods = input("Enter the number of periods for Stock/Forward (default 15): ")
		periods = int(periods)
		opt_periods = input("Enter the nubmer of periods of Options (default 15): ")
		opt_periods = int(opt_periods)
...
u = round(exp((sigma)*sqrt(Tyears/periods)),5)
#u = 1.07
...
q = round((exp((rate-div)*Tyears/periods)-d)/(u-d),4)
...
print("Create the forward lattice")
ForwardLattice = deepcopy(StockLattice)
#k = OptionLattice.keys()
for i in reversed(sorted(ForwardLattice)):
	#print(i)
	#print(OptionLattice[i])
	for j in ForwardLattice[i]:
		if i != len(ForwardLattice)-1:
			#print(str(j.OptionPrice))
			#j.Price = round(max(j.Price-K,0),3)
			#print(str(j.OptionPrice))
		#else:
			#print(str(j.Price) + "; " + str(j.Up.Price) + "; " + str(j.Down.Price))

			j.Price = round((q*j.Up.Price + one_minus_q * j.Down.Price),3)
			#j.Price = round(max())
			#print(str(j.Price))

for i in reversed(sorted(ForwardLattice)):
	print(str(i) + " " * int(i) + str(ForwardLattice[i]))


#Some code to print out the lattice dictionary bottom first
print("Create the call option lattice")
CallOptionLattice = deepcopy(ForwardLattice)

for i in range(opt_periods+1,len(ForwardLattice)):
	CallOptionLattice.pop(i)

for i in range(opt_periods,-1,-1):
	#print(i)
	#print(OptionLattice[i])
	for j in CallOptionLattice[i]:
		if i == opt_periods:
			#print(str(j.OptionPrice))
			j.Price = round(max(j.Price-K,0),3)
			#print(str(j.OptionPrice))
			...
			#print(str(j.Price) + "; " + str(j.Up.Price) + "; " + str(j.Down.Price))
			if (q*j.Up.Price + one_minus_q * j.Down.Price)/exp(rate*(Tyears/periods)) < max(j.Price-K,0):
				print(str(i) + ": Perhaps exercise early")

			j.Price = round(max(max(j.Price-K,0),(q*j.Up.Price + one_minus_q * j.Down.Price)/exp(rate*(Tyears/periods))),3)
			#j.Price = round(max())
			#print(str(j.Price))

for i in reversed(sorted(CallOptionLattice)):
	print(str(i) + " " * int(i) + str(CallOptionLattice[i]))


#Some code to print out the lattice dictionary bottom first
print("Create the put option lattice")
PutOptionLattice = deepcopy(ForwardLattice)

for i in range(opt_periods+1,len(ForwardLattice)):
	PutOptionLattice.pop(i)

#k = OptionLattice.keys()
for i in range(opt_periods,-1,-1):
	#print(i)
	#print(OptionLattice[i])
	for j in PutOptionLattice[i]:
		if i == opt_periods:
			#print(str(j.OptionPrice))
			j.Price = round(max(K-j.Price,0),3)
			#print(str(j.OptionPrice))
		else:
			#print(str(j.Price) + "; " + str(j.Up.Price) + "; " + str(j.Down.Price))
			#if (q*j.Up.Price + one_minus_q * j.Down.Price)/exp(rate*(Tyears/periods)) < max(K-j.Price,0):
			#	print(i)
			j.Price = round(max(max(K-j.Price,0),(q*j.Up.Price + one_minus_q * j.Down.Price)/exp(rate*(Tyears/periods))),3)
			#j.Price = round(max())
			#print(str(j.Price))

for i in reversed(sorted(PutOptionLattice)):
	print(str(i) + " " * int(i) + str(PutOptionLattice[i]))
