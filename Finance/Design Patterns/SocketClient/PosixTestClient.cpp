...
void PosixTestClient::processMessages(vector<MyOrder*> _book)
{
...
void PosixTestClient::placeOrder(vector<MyOrder*> _book)
{
	Contract contract;
	Order order;
	...
	{
		//cout << _book[i]->getSide();
		//*p->getSide();
	


	//contract.symbol = "MSFT";
	contract.symbol = _book[i]->getSymbol();
	contract.secType = "STK";
	contract.exchange = "SMART";
	contract.currency = "USD";

//	order.actionQuantity = QuoteBridgeBuy(

	order.action = _book[i]->getSide();
	order.totalQuantity = _book[i]->getShare();
	order.orderType = "LMT";
	order.lmtPrice = _book[i]->getPrice();

	Logger::Instance()->print( "Placing Order %ld: %s %ld %s at %f\n", m_orderId, order.action.c_str(), order.totalQuantity, contract.symbol.c_str(), order.lmtPrice);
