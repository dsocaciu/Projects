...
euro_K = 84.00
european_lattice = deepcopy(zerocoupon[0:3])

#print(european_lattice)

for i in range(len(european_lattice)-1,-1,-1):
	for j in range(len(european_lattice[i])-1,-1,-1):
		if i == len(european_lattice)-1:
			european_lattice[i][j] = round(max(european_lattice[i][j]-euro_K,0),3)
			#print (str(j))
		else:
			#print(str(i) + " " + str(j) + " " + str(european_lattice[i+1][j+1]) + " " + str(european_lattice[i+1][j]))
			european_lattice[i][j] = round(((european_lattice[i+1][j+1]*q) + (european_lattice[i+1][j]*one_minus_q)) / (1+shortrate[i][j]),5)

print("Printing European Call Lattice")

for i in european_lattice:
	print(i)

print("European Call on Zero Coupon Bond " + str(european_lattice[0][0]))
